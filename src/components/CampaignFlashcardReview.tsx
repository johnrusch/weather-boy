...
export const CampaignFlashcardReview: React.FC<
  CampaignFlashcardReviewProps
> = ({ levelId, onComplete }) => {
  ...
  const fetchFlashcards = async () => {
    try {
      const response = await fetch(
        `/api/campaign-flashcards?levelId=${levelId}`,
      );
  ...
  const handleCardReview = async (mastered: boolean) => {
    ...
    try {
      const response = await fetch(
        `/api/campaign-flashcards?flashcardId=${currentCard._id}`,
        {
          method: "PUT",
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Flashcards_Level${levelId}_${new Date().toISOString().split("T")[0]}.csv`,
    );
    ...
    setDownloadMessage(
      `Successfully downloaded ${selectedFlashcards.length} flashcards`,
    );
    setTimeout(() => setDownloadMessage(null), 3000);
  };
      ...
      <div className="p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mb-4">
            Great job practicing your flashcards.
          </p>
          <p className="text-gray-700">
            Select the flashcards you want to download and click the button
            below.
          </p>
        </div>
