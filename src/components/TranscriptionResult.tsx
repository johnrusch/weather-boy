...
export function TranscriptionResult({ transcription, index }: TranscriptionResultProps) {
  // Get evaluation data and score or provide defaults
  const { score = 0, feedback = 'No feedback available' } = transcription.evaluation || {};
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-6">
      ...
      {/* Transcription */}
      <div className="mb-6">
        <h4 className="text-md font-medium text-gray-700 mb-2">Your response:</h4>
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
          <p className="text-gray-800">{transcription.text || 'No transcription available'}</p>
        </div>
      </div>
      ...
      {transcription.flashcards && transcription.flashcards.length > 0 && (
        <div>
          <h4 className="text-md font-medium text-gray-700 mb-2">Flashcards:</h4>
          <div className="space-y-3">
            {transcription.flashcards.map((card, cardIndex) => (
              <div key={cardIndex} className="bg-blue-50 p-4 rounded-lg border border-blue-200">
