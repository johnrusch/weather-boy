...
export const prompts: PromptWithDifficulty[] = [...frenchPrompts, ...spanishPrompts];
...
export function getPromptsByLanguageAndDifficulty(
  ...
  const languagePrompts = prompts.filter(
    prompt => prompt.language === language && prompt.difficulty === difficulty
  );
...
export function getRandomPromptsByLanguage(
  ...
  const languagePrompts = prompts.filter(prompt => prompt.language === language);
  const shuffled = [...languagePrompts].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, Math.min(count, shuffled.length));
}
