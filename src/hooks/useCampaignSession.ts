...
export function useCampaignSession(): CampaignSessionState {
  ...
  const [levelCompletionMessage, setLevelCompletionMessage] = useState<string | null>(null);
  ...
  const finishCampaignLevel = useCallback(async () => {
    ...
    try {
      ...
      const currentBestScore = campaignState.progress.bestScores[selectedLevel.id] || 0;
    ...
    selectedLevel,
    languageSession.transcriptions,
    campaignState
  ]);
