...
describe('Flashcards API', () => {
  ...
  it('should process correctly formatted French text input', async () => {
    ...
    const frenchText = "Bonjour, je suis allé à le marché hier et j'ai acheté les légumes pour le dîner. Les fruits sont très frais.";
    ...
    const correctionCards = responseBody.flashcards.filter((card: any) => card.type === 'correction');
    const variationCards = responseBody.flashcards.filter((card: any) => card.type === 'variation');
    const translationCards = responseBody.flashcards.filter((card: any) => card.type === 'translation');
  ...
  it('should process correctly formatted Spanish text input', async () => {
    ...
    const spanishText = "Hola, yo fue al mercado ayer y compré las verduras para la cena. Las frutas están muy frescas.";
  ...
  it('should handle API errors gracefully', async () => {
    ...
    const responseBody = await response.json();
    expect(responseBody).toHaveProperty('error', 'Failed to generate flashcards');
  });
