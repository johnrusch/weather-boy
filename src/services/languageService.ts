 ...
 */
export function getStoredLanguage(contextLanguage: string = 'french'): SupportedLanguage {
...
export function changeLanguage(language: SupportedLanguage): void {
  ...
  try {
    ...
    alert(`Error changing language. Please try the debug page at /debug-language`);
  }
}
...
export function debugLanguage(): Record<string, any> {
  ...
  selectors.forEach(selector => {
    selectorValues[(selector as HTMLElement).id] = (selector as HTMLSelectElement).value;
  });
