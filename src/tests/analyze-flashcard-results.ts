...
function analyzeResults(results: TestResults) {
  ...
  console.log(
    `Success Rate: ${successRate.toFixed(2)}% (${successfulTests}/${totalTests})\n`,
  );

  // Group by language
  const frenchResults = testResults.filter(
    (r) => r.testCase.language === "french",
  );
  const spanishResults = testResults.filter(
    (r) => r.testCase.language === "spanish",
  );
  ...
  console.log(
    `- Corrections: ${correctionCards} (${((correctionCards / totalFlashcards) * 100).toFixed(2)}%)`,
  );
  console.log(
    `- Variations: ${variationCards} (${((variationCards / totalFlashcards) * 100).toFixed(2)}%)`,
  );
  console.log(
    `- Translations: ${translationCards} (${((translationCards / totalFlashcards) * 100).toFixed(2)}%)`,
  );
  ...
  testResults.forEach((result, index) => {
    ...
    console.log(
      `${index + 1}. [${success ? "SUCCESS" : "FAILURE"}] ${testCase.id} - ${testCase.description}`,
    );
    ...
    if (!success) {
      ...
      console.log(
        `   Generated ${cards.length} flashcards: ${corrections} corrections, ${variations} variations, ${translations} translations`,
      );
      ...
      if (testCase.expectedIssues && testCase.expectedIssues.length > 0) {
        const correctionTexts = cards
          ...
          .map((c) => ({
            ...
            correction: c.targetLanguage,
          }));

        console.log(
          `   Expected issues: ${testCase.expectedIssues.join(", ")}`,
        );
        console.log(`   Corrections made:`);
        correctionTexts.forEach((corr, i) => {
          console.log(
            `     ${i + 1}. "${corr.original}" → "${corr.correction}"`,
          );
        });
      }
    }
