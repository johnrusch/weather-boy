...
async function testFlashcardGeneration(text: string, language: 'french' | 'spanish'): Promise<Flashcard[]> {
  try {
...
function printFlashcards(flashcards: Flashcard[]) {
  ...
  // Summary by type
  const correctionCards = flashcards.filter(c => c.type === 'correction').length;
  const variationCards = flashcards.filter(c => c.type === 'variation').length;
  const translationCards = flashcards.filter(c => c.type === 'translation').length;
...
async function main() {
  try {
    ...
    if (!textArg) {
      ...
      console.log('Usage: npm run test-custom-flashcards -- --text="Your text here" --language=french');
      process.exit(1);
    }
    
    const text = textArg.split('=')[1].replace(/^"|"$/g, ''); // Remove quotes if present
    const language = (languageArg?.split('=')[1] || 'french') as 'french' | 'spanish';
