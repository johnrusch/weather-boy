...
function analyzeTranscription(
  ...
  testCase: TranscriptionTestCase,
): {
  passed: boolean;
...
async function runTest(testCase: TranscriptionTestCase): Promise<void> {
  ...
  try {
    ...
    if (results.preservedEnglishPhrases.length > 0) {
      console.log(
        "✅ Preserved English phrases:",
        results.preservedEnglishPhrases,
      );
    }
    ...
    if (results.preservedTargetLanguagePhrases.length > 0) {
      console.log(
        "✅ Preserved target language phrases:",
        results.preservedTargetLanguagePhrases,
      );
    }

    if (results.missingTargetLanguagePhrases.length > 0) {
      console.log(
        "❌ Missing target language phrases:",
        results.missingTargetLanguagePhrases,
      );
    }
    ...
    if (results.incorrectlyTranslatedPhrases.length > 0) {
      console.log(
        ...
        results.incorrectlyTranslatedPhrases,
      );
    }
...
async function runCustomTest(
  ...
  try {
    ...
    console.log(
      "NOTE: Custom text testing requires text-to-speech functionality which is not implemented here.",
    );
...
async function main() {
  ...
  if (!fs.existsSync(samplesDir)) {
    ...
    console.log(
      `Please add audio samples to this directory before running tests.`,
    );
  }
...
`);
  ...
  if (argv.language && !argv.text) {
    ...
    if (language !== "french" && language !== "spanish") {
      console.error(
        `Invalid language: ${language}. Must be "french" or "spanish".`,
      );
      return;
    }

    const languageTests = testCases.filter(
